...
function _isNativeApp(){try{return !!(window.Capacitor&&typeof window.Capacitor.isNativePlatform==="function"&&window.Capacitor.isNativePlatform());}catch(e){return false;}}
...
function _renderAccountsLogin() {
  ...
  ct.innerHTML = h;
  if ((_authView === "login" || _authView === "register") && _authCfg && _authCfg.google_client_id) {
    if (_isNativeApp()) _renderGoogleNativeBtn();           // Moya app shell → native plugin
    else _renderGoogleButton(_authCfg.google_client_id);    // web/PWA → Google Identity Services
  }
}
...
// ─── Native Google Sign-In (Capacitor / Moya app shell) ──────────────
// The web GIS button is unreliable inside the Android WebView, so the native
// shell uses the GoogleAuth plugin. It returns an idToken whose `aud` is our
// WEB client id (serverClientId in capacitor.config) — fed to the SAME
// /api/auth/google endpoint the web flow uses (backend checks aud == GOOGLE_CLIENT_ID).
function _gLogoSvg() {
  return '<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true" style="flex-shrink:0">' +
...
async function _googleNativeSignIn() {
  ...
  try { res = await GA.signIn(); }
    ...
    var msg = ((e && (e.message || e.code)) || "").toString();
    if (/cancel/i.test(msg) || /12501/.test(msg)) return;   // user dismissed the chooser
    _authMsg("Google sign-in failed"); return;
  }
  var idToken = res && ((res.authentication && res.authentication.idToken) || res.idToken);
  if (!idToken) { _authMsg("Google sign-in failed"); return; }
  var r = await A("POST", "/api/auth/google", { credential: idToken });
  if (!r || !r.token) { _authMsg(r && r.detail ? r.detail : "Google sign-in failed"); return; }
  _setSess(r.token);
  if (_getSess() !== r.token) { _authMsg("Cannot save login"); return; }
  location.reload();
}
